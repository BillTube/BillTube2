...
$(document).ready(function() {
    ...
    function togglePoster() {
        if (PLAYER.mediaType === 'yt') {
            // Add the 'hidden' class to the .vjs-poster when a YouTube video is playing
            $('.vjs-poster').addClass('hidden');
        } else {
            // Remove the 'hidden' class to show the poster for non-YouTube videos
            $('.vjs-poster').removeClass('hidden');
        }
    }

    // Run when the player is loaded
    $('#ytapiplayer').on('load', function() {
        togglePoster();
    });

    // Also run this when the video changes (e.g., when a new video is loaded)
    $(document).on('changeMedia', function() {
        togglePoster();
    });

    // Additional safeguard: check periodically to ensure the poster remains hidden for YouTube videos
    setInterval(function() {
        togglePoster();
    }, 1000);  // Runs every second to make sure poster stays hidden during YouTube videos
});

ColorsArray = [
    '#ffffff', '#ffefd5', '#c0c0c0', '#808080', '#000000', '#ffff00', '#ffd700', '#ffa500', '#ff4500', '#ff6347',
    '#ffc0cb', '#ff0000', '#dc143c', '#ff1493', '#ff00ff', '#ee82ee', '#9400d3', '#800080', '#a52a2a', '#8b4513',
...
$('#modflair, #emotelistbtn, #voteskip, #fullscreenbtn, #videowrap-header').hide();
$('body').removeClass('compact synchtube hd');
$("#chatwrap").append(
...
var setImageSrc = function(imageData) {
    ...
    var $body = $('body');
    var $userlisttoggle = $('#userlisttoggle');
    ...
    $("#newpollbtn, #leader, #Notif").removeClass("btn btn-sm btn-default");
    ...
    // Right pane and header modifications
    $("#rightpane-inner").prepend("<div id='mediabuttons'></div>");
    $("#nav-collapsible").append("<div id='headermenu' class='headermenu'></div>");
    ...
    // Remove multiple classes from body
    $body.removeClass('compact synchtube hd');
  ...
  var $mainpage = $("#mainpage");
    ...
    // Modify userlist
    $userlist.prepend("<div id='connected'><span id='connectedText'>&nbsp Logged in users</span></div>");
    $userlisttoggle.append($("#usercount"));

    // Add elements
    $navCollapsible.append("<div id='headright'><div id='progbar'></div></div>");
...
if (window.CLIENT.rank >= 3) {
    ...
    // Body modifications
    $("body").addClass("fluid").removeClass("synchtube");

    // Theme and CSS modifications
    ...
    $("#mainpage")
        .append("<div class='nano maincontain' id='maincontain'><div class='nano-content'></div></div>");
    $("#maincontain .nano-content")
        .append($("#mainpage > .container"), $("#mainpage > .container-fluid"));

    // Chat modifications
    $("#messagebuffer").wrap("<div class='nano'></div>");
    $("#chatwrap")
        .children('.form-control')
        .before("<input type='text' id='username' style='width:0;height:0;visibility:hidden;position:absolute;left:0;top:0' /><input type='password' style='width:0;height:0;visibility:hidden;position:absolute;left:0;top:0' />")
        .attr('autocomplete', 'off');
    $(".chat-area-header").after("<div class='nano'></div>");
    $("#chatbox").append($("#chatline"), $("#guestlogin"));
    $(".chat-area-group").append($("#userlisttoggle"));
    $("#chatline").attr('autocomplete', 'off');

    // Navbar and header modifications
    $("nav .navbar-brand").attr('href', document.URL);
    $('#modflair').prependTo('#headermenu');

    // Footer modifications
    $("#sitefooter").append($("#logoutform").removeClass("pull-right"), $('#announcements'));
    ...
    // Remove elements
    $('#videowrap-header, #resizewrap').remove();
...
$(document).ready(function() {
    // Append search input to queue container
    $("#queuecontainer > div").append("<input type='text' class='search form-control pls' placeholder='Name Search'>");

    // Cache DOM elements and create a debounce function
    const $search = $(".search");
    const $queue = $('#queue');
    ...
    // Extend jQuery with case-insensitive contains
    $.extend($.expr[':'], {
    ...
    const performSearch = () => {
        ...
        $queue.children('li').each(function() {
            const $this = $(this);
            const isVisible = searchRegex.test($this.text());
            $this.toggle(isVisible);
            if (isVisible) visibleCount++;
        });

        $counter.text(visibleCount + ' item' + (visibleCount !== 1 ? 's' : ''));
        $noResult.toggle(visibleCount === 0);
    };

    // Attach debounced search to keyup event
    $search.on('keyup', debounce(performSearch, 300));

    // Initial search to set up initial state
    performSearch();
});
...
/* 52 */
/***/ function(module, exports) {
window.cytubeEnhanced.addModule("favouritePictures",function(t){"use strict";var e=this,i=t.parseJSON(window.localStorage.getItem("favouritePictures"),[]);t.storage.setDefault("favouritePictures",_.isArray(i)?i:[]),0===$("#chat-panel").length&&$('<div id="chat-panel" class="row">').insertAfter("#messagebuffer"),0===$("#chat-controls").length&&$('<div id="chat-controls" class="btn-group">').appendTo("#chatwrap"),this.$toggleFavouritePicturesPanelBtn=$('<button id="favourite-pictures-btn" data-tooltip="Favorites" data-tooltip-pos="up" class="chatbtn">').html('<i class="fa-solid fa-folder-heart"></i>'),0!==$("#smiles-btn").length?this.$toggleFavouritePicturesPanelBtn.insertAfter("#smiles-btn"):this.$toggleFavouritePicturesPanelBtn.prependTo("#chat-controls"),this.$favouritePicturesPanel=$('<div id="favourite-pictures-panel">').appendTo("#chat-panel").hide(),this.$favouritePicturesPanelRow=$('<div class="favourite-pictures-panel-row">').appendTo(this.$favouritePicturesPanel),this.$favouritePicturesTrash=$('<div id="pictures-trash" title="'+t.t("favPics[.]Drop the picture here to remove it")+'">').append('<i class="pictures-trash-icon glyphicon glyphicon-trash">').appendTo(this.$favouritePicturesPanelRow),this.$favouritePicturesBodyPanel=$('<div id="pictures-body-panel">').appendTo(this.$favouritePicturesPanelRow),this.$favouritePicturesControlPanel=$('<div id="pictures-control-panel" class="row">').appendTo(this.$favouritePicturesPanel),this.$favouritePicturesControlPanelForm=$('<div class="col-md-12 picmenu">').html('<div class="input-group-pic"><input type="text" id="picture-address" class="form-control input-sm" placeholder="'+t.t("favPics[.]Picture url")+'"><span class="input-group-btn"><button id="add-picture-btn" class="btn btn-sm btn-default fal fa-plus" style="border-radius:2rem;padding:10px;" type="button"></button></span><span class="input-group-btn"><button id="help-pictures-btn" class="btn btn-sm btn-default fas fa-question" style="border-radius:2rem;padding:10px;" type="button"></button></span></div>').appendTo(this.$favouritePicturesControlPanel),this.makeSmilesAndPicturesTogether=function(){e.smilesAndPicturesTogether=!0,e.$toggleFavouritePicturesPanelBtn.hide(),e.$favouritePicturesPanel.hide()},this.entityMap={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},this.replaceUnsafeSymbol=function(t){return e.entityMap[t]},this.renderFavouritePictures=function(){var e=t.storage.get("favouritePictures")||[];this.$favouritePicturesBodyPanel.empty();for(var i=0,a=e.length;i<a;i++){var r=e[i].replace(/[&<>"']/g,this.replaceUnsafeSymbol);$('<img class="favourite-picture-on-panel">').attr({src:r}).appendTo(this.$favouritePicturesBodyPanel)}},this.insertFavouritePicture=function(e){e.startsWith("https://media.giphy.com/media/")&&(e=e.replace("https://media.giphy.com/","https://media0.giphy.com/")),e.endsWith("200_s.gif")&&(e=e.replace(/200_s\.gif$/,"giphy.gif")),t.Helpers.addMessageToChatInput(" "+e+" ","end")},$(document.body).on("click",".favourite-picture-on-panel",function(){e.insertFavouritePicture($(this).attr("src"))}),this.handleFavouritePicturesPanel=function(t){var e=this.smilesAndPicturesTogether||!1;0===$("#smiles-panel").length||e||$("#smiles-panel").hide(),this.$favouritePicturesPanel.toggle(),e||(t.hasClass("btn-default")?(0!==$("#smiles-btn").length&&$("#smiles-btn").hasClass("btn-success")&&($("#smiles-btn").removeClass("btn-success"),$("#smiles-btn").addClass("btn-default")),t.removeClass("btn-default"),t.addClass("")):(t.removeClass("btn-success"),t.addClass("btn-default")))},this.$toggleFavouritePicturesPanelBtn.on("click",function(){e.handleFavouritePicturesPanel($(this))}),this.addFavouritePicture=function(e){if(""!==(e=_.trim(e))){var i=t.storage.get("favouritePictures")||[];if(-1!==i.indexOf(e))return window.makeAlert(t.t("favPics[.]The image already exists")).prependTo(this.$favouritePicturesBodyPanel),$("#picture-address").val(""),!1;""!==e&&i.push(e),$("#picture-address").val(""),t.storage.set("favouritePictures",i),this.renderFavouritePictures()}},$("#add-picture-btn").on("click",function(t){t.preventDefault(),e.addFavouritePicture($("#picture-address").val().trim())}),$("#picture-address").on("keypress",function(t){13==t.which&&e.addFavouritePicture($("#picture-address").val().trim())}),this.showHelp=function(){var e=$('<div class="modal-header__inner">');e.append($('<h3 class="modal-title">').text(t.t("Help")));var i=$('<div class="help-pictures-content">');i.append($("<p>"+t.t("favPics[.]<p>Favourite pictures feature is for saving favourite pictures like browser bookmarks.</p><p>Features:<ul><li><strong>Only links to images can be saved</strong>, so if image from link was removed, it also removes from your panel.</li><li>Image links are stored in your browser.</li><li>Favorite images transfer over to other channels using this theme.</li></ul></p>")+"</p>"));var a=$('<button type="button" id="help-pictures-exit-btn" class="btn btn-info" data-dismiss="modal">'+t.t("favPics[.]Exit")+"</button>"),r=$('<div class="help-pictures-footer">');return r.append(a),t.UI.createModalWindow("chat-history",e,i,r)},$("#help-pictures-btn").on("click",function(t){t.preventDefault(),e.showHelp()}),this.exportPictures=function(){var e="data:text/plain;charset=utf-8,"+encodeURIComponent(t.toJSON(t.storage.get("favouritePictures")||[])),i=$("<a>").attr({href:e,download:"favourite_images.txt"}).hide().appendTo($(document.body));i[0].click(),i.remove()},$("#export-pictures").on("click",function(){e.exportPictures()}),e.mistakeAjaxEnd=!0,$("#upload-pictures").on("click",function(i){i.preventDefault(),e.mistakeAjaxEnd&&(e.mistakeAjaxEnd=!1,$.ajax({type:$(this).attr("method"),url:$(this).attr("action"),dataType:"html",data:{userName:username,userRank:CLIENT.rank,userData:t.storage.get("favouritePictures")||[],g:CLIENT.guest,action:"upload"}}).done(function(i){e.mistakeAjaxEnd=!0,t.UI.createAlertWindow(t.t(`favPics[.]${i}`))}))}),$("#load-pictures").on("click",function(i){i.preventDefault(),e.mistakeAjaxEnd&&(e.mistakeAjaxEnd=!1,$.ajax({type:$(this).attr("method"),url:$(this).attr("action"),dataType:"html",data:{userName:username,userRank:CLIENT.rank,g:CLIENT.guest,action:"get"}}).done(function(i){e.mistakeAjaxEnd=!0;var a=t.parseJSON(i);_.isArray(a)?(t.storage.set("favouritePictures",a),e.renderFavouritePictures()):t.UI.createAlertWindow(t.t(`favPics[.]${i}`))}))}),this.importPictures=function(i){var a=new FileReader;a.addEventListener("load",function(i){var a=t.parseJSON(i.target.result);_.isArray(a)?(t.storage.set("favouritePictures",a),e.renderFavouritePictures()):t.UI.createAlertWindow(t.t("favPics[.]Can't detect any pictures in this file."))}),a.readAsText(i)},$("#import-pictures").on("change",function(){var i=$(this)[0].files[0];t.UI.createConfirmWindow(t.t("favPics[.]Your old pictures will be removed and replaced with the images from the uploaded file (file must correspond to the format of the file from the export button of this panel).<br>Are you sure you want to continue?"),function(t){t&&i&&e.importPictures(i)})}),this.renderFavouritePictures(),this.$favouritePicturesBodyPanel.sortable({containment:this.$favouritePicturesPanelRow,revert:!0,update:function(e,i){var a,r,s=$(i.item).attr("src"),o=$(i.item).next().attr("src"),n=t.storage.get("favouritePictures");-1!==(r=n.indexOf(s))&&(n.splice(r,1),void 0!==o&&-1!==(a=n.indexOf(o))?n.splice(a,0,s):n.push(s),t.storage.set("favouritePictures",n))}}),this.$favouritePicturesTrash.droppable({accept:".favourite-picture-on-panel",hoverClass:"favourite-picture-drop-hover",drop:function(e,i){var a,r=i.draggable.attr("src"),s=t.storage.get("favouritePictures");-1!==(a=s.indexOf(r))&&(s.splice(a,1),t.storage.set("favouritePictures",s)),i.draggable.remove()}})});/***/ },
/* 53 */
/***/ function(module, exports) {
...
function correctSubtitlePlacement() {
    ...
    castScript.src = 'https://dl.dropbox.com/scl/fi/lmubkihemmqf5c35htq4x/billcast.js?rlkey=35h5h0j3xt8xz8t9er4d04cfy&dl=0';
